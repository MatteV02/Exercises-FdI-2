...
void HeapMaxMoveDown(Heap *h, int i) {
    ...
    do {
        ...
        if ((l < (int)h->size) && ElemCompare(HeapGetNodeValue(h, l), HeapGetNodeValue(h, bigger)) > 0) {
            bigger = l;
        }

        if ((r < (int)h->size) && ElemCompare(HeapGetNodeValue(h, r), HeapGetNodeValue(h, bigger)) > 0) {
            bigger = r;
        }

        if (bigger != i) {
            ElemSwap(HeapGetNodeValue(h, i), HeapGetNodeValue(h, bigger));
            i = bigger;
            done = false;
        }
